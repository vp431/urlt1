...
@limiter.exempt
def stats_route():
    # Handle GET request with 'short' or 'short_code' query parameter 
    if request.method == "GET":
        short = request.args.get("short")
        short_code = request.args.get("short_code")
        
        # If either short or short_code is provided, redirect to the proper URL
        if short:
            return redirect(f"/stats/{short}")
        elif short_code:
